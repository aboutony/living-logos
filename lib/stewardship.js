...
import { getLibraryItemById, libraryItemToStream } from "./library.js";
...
export function lightCandle(streamId, amount, donorDid) {
    ...
    // Validate stream exists — try LIVE streams first, then VOD library
    let stream = getStreamById(streamId);
    let contentType = "LIVE";

    if (!stream) {
        // Fallback: check VOD library
        const libraryItem = getLibraryItemById(streamId);
        if (libraryItem) {
            stream = libraryItemToStream(libraryItem);
            // Ensure wallet address is set for parish routing
            stream.walletAddress = stream.walletAddress || `wallet:parish:${stream.id}`;
            contentType = "VOD";
        } else {
            return { success: false, error: `Content not found: ${streamId}` };
        }
    }
    ...
    const transaction = {
        ...
        contentType, // "LIVE" or "VOD" — for Ramez (Moore Group) reconciliation
        status: "confirmed",
